...
public class ContextInstance extends ModuleInstance {
	...
	public void addTransientVariables(Map<String, Object> transientVariables) {
		if (transientVariables != null) {
			Iterator<Map.Entry<String, Object>> iter = transientVariables.entrySet().iterator();
			while (iter.hasNext()) {
				Map.Entry<String, Object> entry = (Map.Entry<String, Object>) iter.next();
				String name = (String) entry.getKey();
				Object value = entry.getValue();
				setTransientVariable(name, value);
			}
		}
	}
	...
	TokenVariableMap createTokenVariableMap(Token token) {
		...
		TokenVariableMap tokenVariableMap = (TokenVariableMap) tokenVariableMaps
				.get(token);
		if (tokenVariableMap == null) {
	...
	public TokenVariableMap getTokenVariableMap(Token token) {
		...
		if (tokenVariableMaps != null) {
			if (tokenVariableMaps.containsKey(token)) {
				tokenVariableMap = (TokenVariableMap) tokenVariableMaps
						.get(token);
			} else if (!token.isRoot()) {
