...
public class OutQtySetupDialog extends TitleAreaDialog {
	...
	public OutQtySetupDialog(Shell parentShell, MovementLine movementLine, Lot lot, Warehouse warehouse) {
		super(parentShell);
...
//						lot.getObjectRrn(), warehouse.getObjectRrn(), Env.getUserRrn()).getQtyOnhand();		
				if(Env.getOrgRrn()==139420L && warehouse.getObjectRrn()==151043L){
					if(lot.getDelayDept()!=null && "自动化A库".equals(lot.getDelayDept())){
						qtyOnhand = invManager.getLotStorageWms(Env.getOrgRrn(),
								lot.getObjectRrn(), warehouse.getObjectRrn(), Env.getUserRrn(),lot);
					}else{
						qtyOnhand = invManager.getLotStorageNew(Env.getOrgRrn(),
								lot.getObjectRrn(), warehouse.getObjectRrn(), Env.getUserRrn(),lot);
					}
				}else{
					qtyOnhand = invManager.getLotStorage(Env.getOrgRrn(),
