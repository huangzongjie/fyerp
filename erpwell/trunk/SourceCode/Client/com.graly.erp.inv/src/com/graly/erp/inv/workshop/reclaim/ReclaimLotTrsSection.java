...
public class ReclaimLotTrsSection extends WorkShopReclaimLineLotSection {
	private static final Logger logger = Logger.getLogger(ReclaimLotTrsSection.class);
	...
	public static final String WorkShop_Delivery = "WorkShopReclaim";
	...
	protected MovementWorkShopReclaim wsReclaim;
	

	public ReclaimLotTrsSection(ADTable adTable, WorkShopReclaimLineLotDialog parentDialog) {
	...
	protected void saveAdapter() {
		try {
			if (validate()) {
				wsReclaim = createMovementTrs();
				List<MovementWorkShopLine> lines = createMovementWorkShopLines();
				INVManager invManager = Framework.getService(INVManager.class);
				wsReclaim = (MovementWorkShopReclaim) invManager.saveMovementWorkShopReclaimLine(wsReclaim, lines,
						Env.getUserRrn());
	...
	protected MovementWorkShopReclaim createMovementTrs() {
		if (wsReclaim != null && wsReclaim.getObjectRrn() != null) {
			if (trsTypeField.getValue() != null) {
				wsReclaim.setTrsType(String.valueOf(trsTypeField.getValue()));
			}
			return wsReclaim;
		}
		MovementWorkShopReclaim trs = new MovementWorkShopReclaim();
		trs.setOrgRrn(Env.getOrgRrn());
		trs.setDocType(MovementWorkShopReclaim.DOCTYPE_DEL);
	...
	protected MovementWorkShopLine generateLine(List<MovementWorkShopLineLot> lineLots,
		...
		line.setLineStatus(MovementWorkShopReclaim.STATUS_DRAFTED);
	...
	protected String getWhereClause() {
		...
		if (this.wsReclaim != null && wsReclaim.getObjectRrn() != null) {
			whereClause.append(" movementRrn = '");
			whereClause.append(this.wsReclaim.getObjectRrn());
			whereClause.append("' ");
	...
	public MovementWorkShopReclaim getWsReclaim() {
		return wsReclaim;
	}

	public void setWsReclaim(MovementWorkShopReclaim wsReclaim) {
		this.wsReclaim = wsReclaim;
	}
