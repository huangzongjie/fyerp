...
public class OtherInLineEntryBlock extends ParentChildEntityBlock {
	Logger logger = Logger.getLogger(OtherInLineEntryBlock.class);
	...
	protected OtherInLineProperties oinLineProperties;
	private String FieldName_Warehouse = "warehouseRrn";
	protected boolean flag = false;

	public OtherInLineEntryBlock(ADTable parentTable, Object parentObject, String whereClause, ADTable childTable, boolean flag) {
	...
	protected void registerPages(DetailsPart detailsPart) {
		try {
			...
			oinLineProperties = new OtherInLineProperties(this, table, getParentObject(), flag);
		...
		try {
			...
			if (parentObject != null) {
				...
				if(in.getPoRrn()!=null){
					...
					po.setObjectRrn(in.getPoRrn());
					parentObject = invManager.approveMovementIn(in, this.getInType(), Env.getUserRrn(), po);
				}else{
					parentObject = invManager.approveMovementIn(in,	this.getInType(), Env.getUserRrn());
				}
		...
		try {
			...
			if (in != null && in.getObjectRrn() != null) {
				...
				if((lines == null || lines.size() == 0) && selectMovementLine == null) 
					return;
				OtherInLotDialog dialog = createInLotDialog(lines);
	...
	protected OtherInLotDialog createInLotDialog(List<MovementLine> lines) {
		return new OtherInLotDialog(UI.getActiveShell(),
