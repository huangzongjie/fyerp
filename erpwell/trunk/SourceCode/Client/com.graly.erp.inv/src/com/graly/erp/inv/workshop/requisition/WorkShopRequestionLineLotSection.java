...
public class WorkShopRequestionLineLotSection extends LotMasterSection {
	private static final Logger logger = Logger.getLogger(WorkShopRequestionLineLotSection.class);
	protected ToolItem itemPrintLot;
	private MovementWorkShopRequestion mt;
	...
	public WorkShopRequestionLineLotSection(WorkShopRequestionLineLotDialog parentDialog, ADTable adTable) {
		super(adTable, parentDialog);
	}
	
	public WorkShopRequestionLineLotSection(WorkShopRequestionLineLotDialog parentDialog, ADTable adTable,
			MovementWorkShopRequestion mt, MovementWorkShopLine transferLine, boolean isView) {
		super(adTable, parentDialog);
		this.mt = mt;
	...
	public WorkShopRequestionLineLotSection(WorkShopRequestionLineLotDialog olld, ADTable adTable,
			MovementWorkShopRequestion mt, MovementWorkShopLine transferLine,
			...
			if(lotId != null && !"".equals(lotId)) {				
				...
				if(validLot(lot)) {
					...
					if(Lot.LOTTYPE_BATCH.equals(lot.getLotType())
						...
						WorkShopLotQtySetupDialog trsQtyDialog = new WorkShopLotQtySetupDialog(UI.getActiveShell(),
								null, lot, wh);
	...
	protected void saveAdapter() {
		try {
			if(validateAll()) {
				INVManager invManager = Framework.getService(INVManager.class);
				mt = (MovementWorkShopRequestion) invManager.saveMovementWorkShopLineLot(mt, lines, Env.getOrgRrn());
				UI.showInfo(Message.getString("common.save_successed"));
				setIsSaved(true);
				((WorkShopRequestionLineLotDialog)parentDialog).buttonPressed(IDialogConstants.CANCEL_ID);
			}
		} catch(Exception e) {
