...
public class WorkShopDeliveryLineEntryBlock extends ParentChildEntityBlock {
	private static final Logger logger = Logger.getLogger(WorkShopDeliveryLineEntryBlock.class);
	protected ToolItem itemOptionalLot;
	protected ToolItem itemApprove;
	protected WorkShopDeliveryLineProperties page;
	...
	public WorkShopDeliveryLineEntryBlock(ADTable parentTable, Object parentObject,
	...
	protected void registerPages(DetailsPart detailsPart) {
		try{
			...
			page = new WorkShopDeliveryLineProperties(this, table, getParentObject(), flag);
	...
	protected void setParenObjectStatusChanged() {
		MovementWorkShopDelivery wsDelivery = (MovementWorkShopDelivery)parentObject;
		String status = "";
		...
		}else if(MovementWorkShopDelivery.STATUS_APPROVED.equals(status)) {
			itemOptionalLot.setEnabled(true);
			itemApprove.setEnabled(false);
		} else if(MovementWorkShopDelivery.STATUS_DRAFTED.equals(status)) {
			itemOptionalLot.setEnabled(true);
			itemApprove.setEnabled(true);
		} else if(MovementWorkShopDelivery.STATUS_CLOSED.equals(status)) {
	...
	protected void setChildObjectStatusChanged() {
		WorkShopDeliveryLineProperties page = (WorkShopDeliveryLineProperties)this.detailsPart.getCurrentPage();
		page.setStatusChanged(((MovementWorkShopDelivery)parentObject).getDocStatus());
	}
	...
	protected void lotAdapter() {
		try {
				...
				parentObject = adManager.getEntity((MovementWorkShopDelivery)parentObject);
				...
				WorkShopDeliveryLineLotDialog od = new WorkShopDeliveryLineLotDialog(UI.getActiveShell(),
	...
	protected void approveAdapter() {
		try {
			...
			if (parentObject != null) {
				...
				MovementWorkShopDelivery mt = (MovementWorkShopDelivery)parentObject;
				INVManager invManager = Framework.getService(INVManager.class);
				parentObject = invManager.approveMovementWorkShopDelivery(mt, Env.getUserRrn());
...
//				this.setParentObject(adManager.getEntity((MovementWorkShopDelivery)parentObject));
	...
	public boolean isEnableByParentObject() {
		MovementWorkShopDelivery out = (MovementWorkShopDelivery)this.getParentObject();
		if(out == null) {
			return false;
		}
		String status = out.getDocStatus();
		if(MovementWorkShopDelivery.STATUS_CLOSED.equals(status)
				|| MovementWorkShopDelivery.STATUS_APPROVED.equals(status)
				|| MovementWorkShopDelivery.STATUS_COMPLETED.equals(status)) {
			return false;
		}
		return true;
	}
