...
public class JSMaterialQtyQuerySection extends QuerySection {
	private Logger logger = Logger.getLogger(JSMaterialQtyQuerySection.class);
	...
	public JSMaterialQtyQuerySection(EntityTableManager tableManager) {
		super(tableManager);
		setWhereClause(" 1 <> 1 ");
	}
...
//				ls = invManager.queryMaterialQtys(Env.getOrgRrn(), materialRrn,"外购","INV.QTY_ONHAND < PMQTY_MIN");
//			}else{
//				ls = invManager.queryMaterialQtys(Env.getOrgRrn(), null,"外购","INV.QTY_ONHAND < PMQTY_MIN");
//			}
	...
	protected void guanlianAdapter(){
		try {
			if(selectedMaterial != null) {
				...
				JSGuanLianDialog cd = new JSGuanLianDialog(UI.getActiveShell(),
						moTable, materialRrn);
