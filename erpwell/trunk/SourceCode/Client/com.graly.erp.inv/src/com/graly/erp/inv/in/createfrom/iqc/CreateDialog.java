...
import com.graly.erp.inv.in.createfrom.po.PoCreateWizard;
import com.graly.erp.inv.in.createfrom.po.PoSelectPage;
import com.graly.erp.inv.in.createfrom.po.PoSelectSection;
 ...
 *
 */
public class CreateDialog extends FlowWizardDialog implements IRefresh{
	private static final Logger LOG = Logger.getLogger(CreateDialog.class);
	...
	public CreateDialog(Shell parentShell, FlowWizard newWizard, TableListManager listTableManager) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		if(wizard instanceof PoCreateWizard){
			((PoCreateWizard)wizard).getContext().setMangedForm(managedForm);
		}
	...
	public void refresh() {
		...
		if(wizard instanceof PoCreateWizard){
			PoSelectPage page = (PoSelectPage)wizard.getPage(PAGE_NAME_PO);
			PoSelectSection section = page.getSection();
			tableViewer = section.getViewer();
		}else if(wizard instanceof IqcCreateWizard){
			IqcSelectPage page = (IqcSelectPage) wizard.getPage(PAGE_NAME_IQC);
			IqcSelectSection section = page.getSection();
			tableViewer = section.getViewer();
		} 	
