...
import com.graly.erp.inv.in.approve.EntityPropertyCellModify;
import com.graly.erp.inv.in.approve.EntityPropertyEdit;
...
public class BJPoLineSelectSection {
	private static final Logger logger = Logger.getLogger(BJPoSelectSection.class);
	
	private BJPoLineSelectPage parentPage;
	...
	public BJPoLineSelectSection(ADTable table, BJPoLineSelectPage parentPage) {
		this.adTable = table;
	...
	protected void createSectionContent(Composite client) {
		try {
	    	...
	    	int length = viewer.getTable().getColumns().length;
	    	CellEditor[] cellEditor = new CellEditor[length];
			String[] properties = new String[length];
			HashMap<String, ADField> proMaps = this.getEditableProperties(tableManager.getADTable());
			for (int i = 0; i < length; i++) {
				String column = (String)viewer.getTable().getColumn(i).getData("qtyDelivered");
				properties[i] = column;
				if("qtyDelivered".equals(column)) {
					cellEditor[i] = new BJModifyTextCellEditor(viewer, column, column, i);
				} else {
					cellEditor[i] = null;
				}
			}
			...
			EntityPropertyCellModify tcm = new EntityPropertyCellModify(viewer,
					new EntityPropertyEdit(proMaps.keySet()));
			viewer.setCellModifier(tcm);
	...
	protected HashMap<String, ADField> getEditableProperties(ADTable editADtable) {
		HashMap<String, ADField> prosMap = new LinkedHashMap<String, ADField>();
		if(editADtable!= null) {
			for(ADField adField : editADtable.getFields()) {
	...
	public void refresh() {
		...
		List list = null;
		ADManager manager =null;
		try {
			manager = Framework.getService(ADManager.class);
            list = manager.getEntityList(Env.getOrgRrn(), input.getTable().getObjectRrn(), 
            		Env.getMaxResult(), input.getWhereClause(), input.getOrderByClause());
        	
//            list = manager.getEntityList(Env.getOrgRrn(), input.getTable().getObjectRrn(), 
//            		Env.getMaxResult(), "1<>1", input.getOrderByClause());

        //收货数为订货数减去收货数:目的是为了可以多次收一个货物
        
        for(Object object : list){
        	BigDecimal qtyDelivered =BigDecimal.ZERO;
        	PurchaseOrderLine poLine = (PurchaseOrderLine) object;
        	List<Movement> movements =manager.getEntityList(Env.getOrgRrn(), Movement.class, 
            		Integer.MAX_VALUE, "poRrn =" +poLine.getPoRrn(), input.getOrderByClause());
        	for(Movement movement : movements){
        		List<MovementLine> movementLines =manager.getEntityList(Env.getOrgRrn(), MovementLine.class, 
                		Integer.MAX_VALUE, "movementRrn =" +movement.getObjectRrn() +" and materialRrn ="+poLine.getMaterialRrn(), input.getOrderByClause());
        			for(MovementLine movementLine : movementLines){
        				BigDecimal movementQty = movementLine.getQtyMovement() !=null? movementLine.getQtyMovement() : BigDecimal.ZERO;
        				qtyDelivered = qtyDelivered.add(movementQty);
            		}
        	}
        	poLine.setQtyDelivered(poLine.getQty().subtract(qtyDelivered));
