...
public class WorkShopReclaimLineEntryBlock extends ParentChildEntityBlock {
	private static final Logger logger = Logger.getLogger(WorkShopReclaimLineEntryBlock.class);
	protected ToolItem itemOptionalLot;
	protected ToolItem itemApprove;
	protected WorkShopReclaimLineProperties page;
	
	private MovementWorkShopLine selectedWSLine;
	private String FieldName_TargetWarehouse = "targetWarehouseRrn";
	private boolean flag = false;
	public static final String KEY_WS_APPROVED = "INV.WorkShopReclaim.Approved";
	
	public WorkShopReclaimLineEntryBlock(ADTable parentTable, Object parentObject,
	...
	protected void registerPages(DetailsPart detailsPart) {
		try{
			...
			page = new WorkShopReclaimLineProperties(this, table, getParentObject(), flag);
	...
	protected void setParenObjectStatusChanged() {
		MovementWorkShopReclaim wsDelivery = (MovementWorkShopReclaim)parentObject;
		String status = "";
		...
		}else if(MovementWorkShopReclaim.STATUS_APPROVED.equals(status)) {
			itemOptionalLot.setEnabled(true);
			itemApprove.setEnabled(false);
		} else if(MovementWorkShopReclaim.STATUS_DRAFTED.equals(status)) {
			itemOptionalLot.setEnabled(true);
			itemApprove.setEnabled(true);
		} else if(MovementWorkShopReclaim.STATUS_CLOSED.equals(status)) {
	...
	protected void setChildObjectStatusChanged() {
		WorkShopReclaimLineProperties page = (WorkShopReclaimLineProperties)this.detailsPart.getCurrentPage();
		page.setStatusChanged(((MovementWorkShopReclaim)parentObject).getDocStatus());
	}
	...
	protected void lotAdapter() {
		try {
				...
				parentObject = adManager.getEntity((MovementWorkShopReclaim)parentObject);
				...
				WorkShopReclaimLineLotDialog od = new WorkShopReclaimLineLotDialog(UI.getActiveShell(),
	...
	protected void approveAdapter() {
		try {
			...
			if (parentObject != null) {
				...
				MovementWorkShopReclaim mt = (MovementWorkShopReclaim)parentObject;
				INVManager invManager = Framework.getService(INVManager.class);
				parentObject = invManager.approveMovementWorkShopReclaim(mt, Env.getUserRrn());
				// 无需用adManager再获得parentObject，是因为itemApprove受状态控制，不能再次审核
				ADManager adManager = Framework.getService(ADManager.class);
				parentObject = adManager.getEntity((ADBase) parentObject);
//				this.setParentObject(adManager.getEntity((MovementWorkShopReclaim)parentObject));
	...
	public boolean isEnableByParentObject() {
		MovementWorkShopReclaim out = (MovementWorkShopReclaim)this.getParentObject();
		if(out == null) {
			return false;
		}
		String status = out.getDocStatus();
		if(MovementWorkShopReclaim.STATUS_CLOSED.equals(status)
				|| MovementWorkShopReclaim.STATUS_APPROVED.equals(status)
				|| MovementWorkShopReclaim.STATUS_COMPLETED.equals(status)) {
			return false;
		}
		return true;
	}
