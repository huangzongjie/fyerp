...
public class WorkShopReqLotTrsSection extends WorkShopRequestionLineLotSection {
	private static final Logger logger = Logger.getLogger(WorkShopReqLotTrsSection.class);
	...
	public static final String WorkShop_Requestion = "WorkShopRequestion";
	...
	protected MovementWorkShopRequestion wsRequestion;
	

	public WorkShopReqLotTrsSection(ADTable adTable, WorkShopRequestionLineLotDialog parentDialog) {
	...
	protected void createPreWarehouseContent(Composite parent,
		...
		String whereClause = " warehouseId = '制造车间良品' ";
		...
		preRefField.setSelectionIndex(0);
	}
	...
	protected void createTrsTypeContent(Composite parent, FormToolkit toolkit)
		...
		String whereClause = " referenceName = '" + WorkShop_Requestion + "'";
		refTable.setWhereClause(whereClause);
	...
	protected void saveAdapter() {
		try {
			if (validate()) {
				wsRequestion = createMovementTrs();
				List<MovementWorkShopLine> lines = createMovementWorkShopLines();
				INVManager invManager = Framework.getService(INVManager.class);
				wsRequestion = invManager.saveMovementWorkShopRequestionLine(wsRequestion, lines,
						Env.getUserRrn());
	...
	protected MovementWorkShopRequestion createMovementTrs() {
		if (wsRequestion != null && wsRequestion.getObjectRrn() != null) {
			if (trsTypeField.getValue() != null) {
				wsRequestion.setTrsType(String.valueOf(trsTypeField.getValue()));
			}
			return wsRequestion;
		}
		MovementWorkShopRequestion trs = new MovementWorkShopRequestion();
		trs.setOrgRrn(Env.getOrgRrn());
		trs.setDocType(MovementWorkShopRequestion.DOCTYPE_MAN);
	...
	protected MovementWorkShopLine generateLine(List<MovementWorkShopLineLot> lineLots,
		...
		line.setLineStatus(MovementWorkShopRequestion.STATUS_DRAFTED);
	...
	protected String getWhereClause() {
		...
		if (this.wsRequestion != null && wsRequestion.getObjectRrn() != null) {
			whereClause.append(" movementRrn = '");
			whereClause.append(this.wsRequestion.getObjectRrn());
			whereClause.append("' ");
	...
	public MovementWorkShopRequestion getWsRequestion() {
		return wsRequestion;
	}

	public void setWsRequestion(MovementWorkShopRequestion wsRequestion) {
		this.wsRequestion = wsRequestion;
	}
