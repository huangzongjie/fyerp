...
public class WorkShopRequestionLineProperties extends ChildEntityProperties {
	private static final Logger logger = Logger.getLogger(WorkShopRequestionLineProperties.class);
	...
	public WorkShopRequestionLineProperties() {
		super();
	}
	
	public WorkShopRequestionLineProperties(EntityBlock masterParent, ADTable table, Object parentObject) {
		this(masterParent, table, parentObject, false);
	}
	
	public WorkShopRequestionLineProperties(EntityBlock masterParent, ADTable table, Object parentObject, boolean flag) {
...
//		}
	...
	public void refresh() {
		super.refresh();
		WorkShopRequestionLineEntryBlock block = (WorkShopRequestionLineEntryBlock)getMasterParent();
		...
		try {
			...
			if (getAdObject() != null) {
				...
				if (saveFlag) {
					...
					MovementWorkShopRequestion mm = (MovementWorkShopRequestion)block.getParentObject();
					if(isContainsSameMaterialInLines(mm, line)) {
//						newAdapter();
						return;
					}
					// 获得该Line对应的Lots
					line.setMovementWorkShopLots(getLineLotList(mm, line));

					// 调用savePOLine()方法
					line = invManager.saveMovementWorkShopRequestionLine(mm, line, Env.getUserRrn());
	...
	protected boolean isContainsSameMaterialInLines(MovementWorkShopRequestion mm, MovementWorkShopLine line) {
	...
	protected List<MovementWorkShopLineLot> getLineLotList(MovementWorkShopRequestion mtrs, MovementWorkShopLine line) throws Exception{
    	...
    	lineLots = manager.getEntityList(Env.getOrgRrn(), MovementWorkShopLineLot.class,
        		Integer.MAX_VALUE, whereClause, null);
		return lineLots;
	}
	
	protected void deleteAdapter() {
		if(!MovementWorkShopRequestion.STATUS_DRAFTED.equals( ((MovementWorkShopLine)getAdObject()).getLineStatus()) ) {
			return;
		}
		try {
			...
			if (getAdObject().getObjectRrn() != null) {
				if (confirmDelete) {
					INVManager invManager = Framework.getService(INVManager.class);
					invManager.deleteMovementWorkShopLine((MovementWorkShopLine)getAdObject(), false,Env.getUserRrn());
					// 刷新父对象
					WorkShopRequestionLineEntryBlock block = (WorkShopRequestionLineEntryBlock)getMasterParent();
					MovementWorkShopRequestion mm = (MovementWorkShopRequestion)block.getParentObject();
					ADManager adManager = Framework.getService(ADManager.class);
	...
	public ADBase createAdObject() throws Exception {
		...
		try {
			WorkShopRequestionLineEntryBlock block = (WorkShopRequestionLineEntryBlock)this.getMasterParent();
			if(block.isEnableByParentObject()) {
				INVManager invManager = Framework.getService(INVManager.class);			
				wsLine = invManager.newMovementWorkShopLine((MovementWorkShopRequestion)block.getParentObject());
			} else {
