...
public class AgingBTzzDialog extends EntityQueryDialog {

	public AgingBTzzDialog(Shell parent, EntityTableManager tableManager, IRefresh irefresh) {
...
//			iRefresh.refresh();
			...
			String report = "aging_bt_zz.rptdesign";
			...
			for(String name : keys.keySet()){
				...
				if("materialType".equals(name)){
					materialType =  (String) keys.get(name);
				}
			...
			if(from==null || to==null || materialType==null ){
				UI.showError("开始和结束时间与物料分类不能为空");
				return;
			}
			userParams.put("START_DATE", from);
			userParams.put("END_DATE", to);
			userParams.put("MATERIAL_TYPE", materialType);
