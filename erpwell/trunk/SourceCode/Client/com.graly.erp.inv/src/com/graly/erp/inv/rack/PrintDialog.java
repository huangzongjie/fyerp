...
public class PrintDialog extends ExtendDialog{
	...
	public PrintDialog(List<WarehouseRack> racks) {
		super();
		this.racks = racks;
	}
	
	public PrintDialog(WarehouseRack rack) {
		racks = new ArrayList<WarehouseRack>();
		racks.add(rack);		
	}
		...
		try {
			if (IDialogConstants.OK_ID == buttonId) {
				if(racks != null && racks.size() != 0){
					...
					RackPrintProgressDialog progressDialog = new RackPrintProgressDialog(UI.getActiveShell());
					RackPrintProgress progress = new RackPrintProgress(racks, getRepeatTime(), doublePrint.getSelection());
