...
public class TransferToCanaLineEntryBlock extends ParentChildEntityBlock {
	private static final Logger logger = Logger.getLogger(TransferToCanaLineEntryBlock.class);
	protected ToolItem itemOptionalLot;
	protected ToolItem itemApprove;
	protected ToolItem itemPreview;
	protected TransferToCanaLineProperties page;
	...
	public TransferToCanaLineEntryBlock(ADTable parentTable, Object parentObject,
	...
	protected void createParentContent(Composite client) {
		...
		for (ADTab tab : parentTable.getTabs()) {
			...
			TransferForm itemForm = new TransferForm(getTabs(), SWT.NONE, tab, mmng);
			RefTableField rf = (RefTableField) itemForm.getFields().get("targetWarehouseRrn");
			if(rf!=null){
				rf.setSelectionIndex(0);
				rf.getComboControl().select(0);
				rf.setValue("42803113");
				rf.refresh();
			}
	...
	protected void registerPages(DetailsPart detailsPart) {
		try{
			...
			page = new TransferToCanaLineProperties(this, table, getParentObject(), flag);
	...
	protected void setChildObjectStatusChanged() {
		TransferToCanaLineProperties page = (TransferToCanaLineProperties)this.detailsPart.getCurrentPage();
...
//		});
	...
	protected void lotAdapter() {
		try {
				...
				TransferToCanaLineLotDialog od = new TransferToCanaLineLotDialog(UI.getActiveShell(),
	...
	protected void approveAdapter() {
		try {
			...
			if (parentObject != null) {
				...
				parentObject = invManager.approveMovementTransferToCana(mt, Env.getUserRrn());
