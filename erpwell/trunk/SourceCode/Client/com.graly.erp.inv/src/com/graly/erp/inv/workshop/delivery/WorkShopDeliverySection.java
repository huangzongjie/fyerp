...
public class WorkShopDeliverySection extends MasterSection {
	...
	private MovementWorkShopDelivery selectedWSDelivery;
	...
	public WorkShopDeliverySection(EntityTableManager tableManager){
		super(tableManager);
	}
	...
	protected void lotDeliveryAdapter() {
		WorkShopDelLotTrsDialog olbd = new WorkShopDelLotTrsDialog(UI.getActiveShell());
		if(olbd.open() == Dialog.CANCEL) {
			MovementWorkShopDelivery newTrs = ((WorkShopDelLotTrsSection)olbd.getLotMasterSection()).getWsDelivery();
	...
	protected void newAdapter() {
		...
		MovementWorkShopDelivery wsDelivery = new MovementWorkShopDelivery();
		wsDelivery.setOrgRrn(Env.getOrgRrn());
		wsDelivery.setDocType(MovementWorkShopDelivery.DOCTYPE_DEL);
		wsDelivery.setUserCreated(Env.getUserName());
		
		WorkShopDeliveryLineDialog olbd = new WorkShopDeliveryLineDialog(UI.getActiveShell(),
				this.getTableManager().getADTable(), whereClause, wsDelivery, getADTableOfMovementLine());
		if(olbd.open() == Dialog.CANCEL) {
			wsDelivery = (MovementWorkShopDelivery)olbd.getParentObject();
	...
	protected void editAdapter() {
		try {
			if(selectedWSDelivery != null && selectedWSDelivery.getObjectRrn() != null) {
				...
				selectedWSDelivery = (MovementWorkShopDelivery)adManager.getEntity(selectedWSDelivery);
				String whereClause = ( " movementRrn = " + selectedWSDelivery.getObjectRrn().toString() + " ");
				WorkShopDeliveryLineDialog cd = new WorkShopDeliveryLineDialog(UI.getActiveShell(),
...
//			refresh();
	...
	private void setSelectionMovementOut(Object obj) {
		if(obj instanceof MovementWorkShopDelivery) {
			selectedWSDelivery = (MovementWorkShopDelivery)obj;
