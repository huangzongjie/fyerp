...
public class OtherInLotSection extends LotMasterSection {
	private static final Logger logger = Logger.getLogger(OtherInLotSection.class);
	...
	public OtherInLotSection(ADTable adTable, LotDialog parentDialog) {
		super(adTable, parentDialog);
	}

	public OtherInLotSection(ADTable adTable, MovementIn in,
	...
	protected void setEnableByStatus() {
		...
		if(isView || (in != null && MovementIn.STATUS_APPROVED.equals(in.getDocStatus()))) {
			setEnable(false);
			setPrintEnable(true);
			((OtherInLotTableManager)lotManager).setCanEdit(false);
		}
//		else if(in != null && MovementIn.STATUS_DRAFTED.equals(in.getDocStatus())) {
//			((OtherInLotTableManager)lotManager).setCanEdit(true);
//		}
	}
	...
	protected void saveAdapter() {
		try {
			if(lines != null && lines.size() > 0) {
				if(validateAll()) {
					INVManager invManager = Framework.getService(INVManager.class);
					in = invManager.saveMovementInLine(in, lines, MovementIn.InType.OIN, Env.getUserRrn());
