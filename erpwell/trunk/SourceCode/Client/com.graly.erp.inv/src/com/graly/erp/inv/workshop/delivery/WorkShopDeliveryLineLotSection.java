...
public class WorkShopDeliveryLineLotSection extends LotMasterSection {
	private static final Logger logger = Logger.getLogger(WorkShopDeliveryLineLotSection.class);
	protected ToolItem itemPrintLot;
	private MovementWorkShopDelivery mt;
	private MovementWorkShopLine transferLine;
	
	private List<MovementWorkShopLineLot> lineLots;
	private MovementWorkShopLineLot selectedLineLot;
	...
	public WorkShopDeliveryLineLotSection(WorkShopDeliveryLineLotDialog parentDialog, ADTable adTable) {
		super(adTable, parentDialog);
	}
	
	public WorkShopDeliveryLineLotSection(WorkShopDeliveryLineLotDialog parentDialog, ADTable adTable,
			MovementWorkShopDelivery mt, MovementWorkShopLine transferLine, boolean isView) {
		super(adTable, parentDialog);
		this.mt = mt;
	...
	public WorkShopDeliveryLineLotSection(WorkShopDeliveryLineLotDialog olld, ADTable adTable,
			MovementWorkShopDelivery mt, MovementWorkShopLine transferLine,
	...
	protected void setItemInitStatus() {
		if (isView || (transferLine != null && !MovementWorkShop.STATUS_DRAFTED.equals(transferLine.getLineStatus()))) {
...
//		createToolItemPrintLot(tBar);
	...
	protected void printLotAdapter() {
		try {
			this.lineLots = (List<MovementWorkShopLineLot>)viewer.getInput();
			if(lineLots != null && lineLots.size() != 0){
				List<Lot> lots = new ArrayList<Lot>();
				for(MovementWorkShopLineLot lineLot : lineLots) {
					Lot lot = new Lot();
	...
	protected void setMovementWorkShopLineSelect(Object obj) {
		if (obj instanceof MovementWorkShopLineLot) {
			selectedLineLot = (MovementWorkShopLineLot) obj;
		} else {
			...
			if(lotId != null && !"".equals(lotId)) {				
				...
				if(validLot(lot)) {
					...
					if(Lot.LOTTYPE_BATCH.equals(lot.getLotType())
						...
						DeliveryLotQtySetupDialog trsQtyDialog = new DeliveryLotQtySetupDialog(UI.getActiveShell(),
								null, lot, wh);
	...
	protected void saveAdapter() {
		try {
			if(validateAll()) {
				INVManager invManager = Framework.getService(INVManager.class);
				mt = (MovementWorkShopDelivery) invManager.saveMovementWorkShopLineLot(mt, lines, Env.getOrgRrn());
				UI.showInfo(Message.getString("common.save_successed"));
				setIsSaved(true);
				((WorkShopDeliveryLineLotDialog)parentDialog).buttonPressed(IDialogConstants.CANCEL_ID);
			}
		} catch(Exception e) {
	...
	protected boolean validateAll() {
		...
		List<MovementWorkShopLineLot> lLots = null;
		...
		for(MovementWorkShopLine line : lines) {
			...
			for(MovementWorkShopLineLot linelot : lLots) {
			...
			if(total.compareTo(line.getQtyMovement()) == 0) {
				line.setMovementWorkShopLots(lLots);
				continue;
			} else {
				line.setMovementWorkShopLots(lLots);
	...
	/* 删除MovementWorkShopLineLot列表中选中的MovementWorkShopLineLot*/
...
//			}
	...
	protected MovementWorkShopLineLot pareseMovementWorkShopLineLot(MovementWorkShopLine line,
			BigDecimal outQty, Lot lot, boolean isGenNewLotId) throws Exception {
		Date now = Env.getSysDate();
		MovementWorkShopLineLot trsLineLot = new MovementWorkShopLineLot();
...
//			whereClause.append(" MovementWorkShopLineRrn = '");
...
//		} else 
	...
	public List<MovementWorkShopLineLot> getLineLots() {
		if(lineLots == null) {
