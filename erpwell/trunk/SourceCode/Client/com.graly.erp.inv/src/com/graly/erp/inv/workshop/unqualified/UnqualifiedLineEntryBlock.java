...
public class UnqualifiedLineEntryBlock extends ParentChildEntityBlock {
	private static final Logger logger = Logger.getLogger(UnqualifiedLineEntryBlock.class);
	protected ToolItem itemOptionalLot;
	protected ToolItem itemApprove;
	protected UnqualifiedLineProperties page;
	...
	public UnqualifiedLineEntryBlock(ADTable parentTable, Object parentObject,
	...
	protected void registerPages(DetailsPart detailsPart) {
		try{
			...
			page = new UnqualifiedLineProperties(this, table, getParentObject(), flag);
	...
	protected void setParenObjectStatusChanged() {
		MovementWorkShopUnqualified wsDelivery = (MovementWorkShopUnqualified)parentObject;
		String status = "";
		...
		}else if(MovementWorkShopUnqualified.STATUS_APPROVED.equals(status)) {
			itemOptionalLot.setEnabled(true);
			itemApprove.setEnabled(false);
		} else if(MovementWorkShopUnqualified.STATUS_DRAFTED.equals(status)) {
			itemOptionalLot.setEnabled(true);
			itemApprove.setEnabled(true);
		} else if(MovementWorkShopUnqualified.STATUS_CLOSED.equals(status)) {
	...
	protected void setChildObjectStatusChanged() {
		UnqualifiedLineProperties page = (UnqualifiedLineProperties)this.detailsPart.getCurrentPage();
		page.setStatusChanged(((MovementWorkShopUnqualified)parentObject).getDocStatus());
	}
	...
	protected void lotAdapter() {
		try {
				...
				parentObject = adManager.getEntity((MovementWorkShopUnqualified)parentObject);
				...
				UnqualifiedLineLotDialog od = new UnqualifiedLineLotDialog(UI.getActiveShell(),
	...
	protected void approveAdapter() {
		try {
			...
			if (parentObject != null) {
				...
				MovementWorkShopUnqualified mt = (MovementWorkShopUnqualified)parentObject;
				INVManager invManager = Framework.getService(INVManager.class);
				parentObject = invManager.approveMovementTransferUnqualified(mt, Env.getUserRrn());
				// 无需用adManager再获得parentObject，是因为itemApprove受状态控制，不能再次审核
				ADManager adManager = Framework.getService(ADManager.class);
				parentObject = adManager.getEntity((ADBase) parentObject);
//				this.setParentObject(adManager.getEntity((MovementWorkShopUnqualified)parentObject));
	...
	public boolean isEnableByParentObject() {
		MovementWorkShopUnqualified out = (MovementWorkShopUnqualified)this.getParentObject();
		if(out == null) {
			return false;
		}
		String status = out.getDocStatus();
		if(MovementWorkShopUnqualified.STATUS_CLOSED.equals(status)
				|| MovementWorkShopUnqualified.STATUS_APPROVED.equals(status)
				|| MovementWorkShopUnqualified.STATUS_COMPLETED.equals(status)) {
			return false;
		}
		return true;
	}
