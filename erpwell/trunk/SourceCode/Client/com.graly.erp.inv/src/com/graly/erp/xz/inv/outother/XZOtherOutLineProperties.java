...
public class XZOtherOutLineProperties extends OutLineProperties {
	private static final Logger logger = Logger.getLogger(XZOtherOutLineProperties.class);
	private String PREFIX = " movementRrn = '";
	
	public XZOtherOutLineProperties() {
		super();
    }
	
	public XZOtherOutLineProperties(EntityBlock masterParent, ADTable table, Object parentObject,boolean flag) {
	...
	public void refresh() {
		super.refresh();
		XZOtherOutLineEntryBlock block = (XZOtherOutLineEntryBlock)getMasterParent();
		block.setParenObjectStatusChanged();
	}

	/**
	 * 为防止浪费多余的字段，因此选择用设备字段来记录人员名称和部门
	 * */
		...
		try {
			...
			if (getAdObject() != null) {
				...
				if (saveFlag) {
					...
					//行政equipmentRrn记录领用人和
					MovementLine line = (MovementLine)getAdObject();
					line.setLocatorId(MovementOut.DOCTYPE_OOU);//用来记录是出库还是入库
					if(line.getEquipmentRrn()!=null){
						ADUser user = new ADUser();
						user.setObjectRrn(line.getEquipmentRrn());
						ADManager adManager = Framework.getService(ADManager.class);
						user = (ADUser) adManager.getEntity(user);
						if(user !=null ){
							line.setEquipmentName(user.getDescription());//人员姓名
							if(user.getDepartment()!=null && !"".equals(user.getDepartment().trim())){
								line.setEquipmentId(user.getDepartment());//部门
							}
