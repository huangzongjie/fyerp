...
public class TransferLineLotSection extends LotMasterSection {
	private static final Logger logger = Logger.getLogger(TransferLineLotSection.class);
	...
	public TransferLineLotSection(TransferLineLotDialog parentDialog, ADTable adTable) {
	...
	public TransferLineLotSection(TransferLineLotDialog olld, ADTable adTable,
...
//				lot = invManager.getLotByLotId(Env.getOrgRrn(), lotId);
				if(Env.getOrgRrn() == 139420L){
					if(mt!=null && mt.getWmsWarehouse()!=null&& mt.getWmsWarehouse().length()>0){
						//只能取WMS库中的批次
				...
				if(validLot(lot)) {
					...
					if(Lot.LOTTYPE_BATCH.equals(lot.getLotType())
						...
						if(mt!=null){//WMS添加
							lot.setDelayDept(mt.getWmsWarehouse());
						}
	...
	protected void saveAdapter() {
		try {
			if(validateAll()) {
				...
				setPrintabled(true);
				setIsSaved(true);
				((TransferLineLotDialog)parentDialog).buttonPressed(IDialogConstants.CANCEL_ID);
			}
		} catch(Exception e) {
...
//		}
	...
	protected MovementLineLot pareseMovementLineLot(MovementLine line,
		...
		if(Env.getOrgRrn() ==139420L){
			trsLineLot.setReverseField7(lot.getReverseField7());
			trsLineLot.setReverseField8(lot.getReverseField8());
			trsLineLot.setReverseField9(lot.getReverseField9());
		}
		return trsLineLot;
	}
