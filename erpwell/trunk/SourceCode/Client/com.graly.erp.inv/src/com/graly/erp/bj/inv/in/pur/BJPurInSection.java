...
public class BJPurInSection extends MasterSection {

	private static final Logger logger = Logger.getLogger(BJPurInSection.class);
	...
	private static final String TABLE_NAME_MOVEMENTLINE = "INVMovementLine";
	...
	private Menu menu;
	protected static String PAGE_CATEGORY_PO = "createPo";
	protected static String PAGE_CATEGORY_IQC = "createIqc";
	
	public BJPurInSection(EntityTableManager tableManager) {
		super(tableManager);
	}
	...
	protected void menuPoAdapter() {
		...
		ADTable table = context.getTable(CreateContext.TableName_Po);
		TableListManager listTableManager = new TableListManager(table);
		
		PoCreateWizard wizard = new PoCreateWizard(context, PAGE_CATEGORY_PO);
		CreateDialog dialog = new CreateDialog(UI.getActiveShell(), wizard, listTableManager);
		context.setDialog(dialog);
		int code = dialog.open();
		if (code == Dialog.OK) {
	...
	protected void editAdapter() {
		try {
			if(selectedIn != null && selectedIn.getObjectRrn() != null) {
				...
				BJPurInLineDialog inLineDialog = new BJPurInLineDialog(UI.getActiveShell(), this.getTableManager().getADTable(), whereClause, selectedIn,
						adTable, false);
