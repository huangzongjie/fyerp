...
public class XZOtherOutSection extends OutSection {	
	private static final Logger logger = Logger.getLogger(XZOtherOutSection.class);
	private static final String TABLE_NAME = "XZINVMovementOutOtherLine";
	...
	public XZOtherOutSection(EntityTableManager tableManager){
		super(tableManager);
	}
...
//		createToolItemByLotOut(tBar);
	...
	protected void newAdapter() {
		...
		if(wareHouses!=null && wareHouses.size() >0){
			out.setWarehouseRrn(wareHouses.get(0).getObjectRrn());
		}
		
		XZOtherOutLineBlockDialog olbd = new XZOtherOutLineBlockDialog(UI.getActiveShell(),
	...
	protected void editAdapter() {
		try {
			if(selectedOut != null && selectedOut.getObjectRrn() != null) {
				...
				XZOtherOutLineBlockDialog cd = new XZOtherOutLineBlockDialog(UI.getActiveShell(),
				...
				if (confirmDelete) {
					if (selectedOut.getObjectRrn() != null) {
						if(DelInvMovementAuthorityManager.hasDeleteAuthority(Env.getUserRrn(),
							...
							ADManager adManager = Framework.getService(ADManager.class);
							Requisition requistion = new Requisition();
							requistion.setObjectRrn(selectedOut.getIqcRrn());
							requistion = (Requisition) adManager.getEntity(requistion);
							requistion.setDescription(null);
							adManager.saveEntity(requistion, Env.getOrgRrn());
