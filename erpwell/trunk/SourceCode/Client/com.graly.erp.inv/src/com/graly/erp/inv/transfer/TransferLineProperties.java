...
public class TransferLineProperties extends ChildEntityProperties {
	private static final Logger logger = Logger.getLogger(TransferLineProperties.class);
	...
	public TransferLineProperties() {
		super();
	}
	
	public TransferLineProperties(EntityBlock masterParent, ADTable table, Object parentObject) {
		this(masterParent, table, parentObject, false);
	}
	
	public TransferLineProperties(EntityBlock masterParent, ADTable table, Object parentObject, boolean flag) {
	...
	protected void createSectionContent(Composite client) {
		...
		//Locator第一次赋值
		TransferLineEntryBlock block = (TransferLineEntryBlock)getMasterParent();
	...
	protected void saveAdapter() {
		try {
			...
			if (getAdObject() != null) {
				...
				if(Env.getOrgRrn() == 139420L){//自动化车间校验
					MovementTransfer trf = (MovementTransfer) getParentObject();
					if(trf !=null&&trf.getWmsWarehouse()!=null&&!"".equals(trf.getWmsWarehouse()) ){
						String whouse = trf.getWarehouseRrn()==null?"":trf.getWarehouseRrn().toString();
						if(!"151043".equals(whouse)){
							UI.showError("自动化A库原仓库必须选择环保-良品");
							return;
						}
					}
				}
				...
				if (saveFlag) {
					...
					if(Env.getOrgRrn() ==139420L && mm.getWarehouseRrn()!=null && mm.getWarehouseRrn()==151043L){
						Storage storage =invManager.getMaterialStorage(Env.getOrgRrn(),  line.getMaterialRrn(), 151043L, Env.getUserRrn());
						if(storage==null){
							UI.showError("库存不足不允许调拨");  
							return;
						}else{
							BigDecimal qtyOnhand = storage.getQtyOnhand()!=null?storage.getQtyOnhand():BigDecimal.ZERO;
							if(line.getQtyMovement().compareTo(qtyOnhand)>0){
								UI.showError("库存不足不允许调拨");  
								return;
							}
						}
...
//						newAdapter();
					...
					// 调用savePOLine()方法
					line = invManager.saveMovementTransferLine(mm, line, Env.getUserRrn());
	...
	public ADBase createAdObject() throws Exception {
		...
		try {
			TransferLineEntryBlock block = (TransferLineEntryBlock)this.getMasterParent();
