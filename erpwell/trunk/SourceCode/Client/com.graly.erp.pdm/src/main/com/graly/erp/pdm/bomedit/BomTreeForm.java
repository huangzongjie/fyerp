...
public class BomTreeForm extends Form {
	...
	protected BomTreeDialog btd;
	protected MaterialTreeManager treeManager;
	protected TreeViewer viewer;
	protected Long version;
	
	public BomTreeForm(Composite parent, int style, Object object) {
    	super(parent, style, object);
    }
	
	public BomTreeForm(Composite parent, int style, ADBase adBase,
			IMessageManager mmng, BomTreeDialog btd) {
...
//			viewer.addSelectionChangedListener(getSelectionChangedListener());
		...
		protected void selectAdapter(SelectionEvent e) {
			...
			if(selectedItem.getParentItem() != null) {
				parent = selectedItem.getParentItem().getData();
			}
			if(obj instanceof Bom) {
				if(BomConstant.isContains((Bom)obj) && parent instanceof Material) {
					btd.setActiveBom((Bom)obj);
					Material mr = (Material)parent;
					btd.setEnableOptional((Bom)obj, mr.getObjectRrn());
				} else {
					btd.setActiveBom(null);
					btd.setEnableOptional(null, null);
				}
				// 只要是Bom, 就可以设置可替代料
				btd.setEnableAlter((Bom)obj, getPath(selectedItem));
			} else {
				btd.setActiveBom(null);
				btd.setEnableOptional(null, null);
				btd.setEnableAlter(null, null);
			}
		}
