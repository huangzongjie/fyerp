...
public class MpsLineGenerateProgress implements IRunnableWithProgress {
	private static final Logger logger = Logger.getLogger(MpsLineGenerateProgress.class);
	...
	public MpsLineGenerateProgress(Mps mps, MpsEntityBlock parentBlock) {
		this.mps = mps;
...
//			ppmManager.generateMaterialBatchSum(Env.getOrgRrn());
			...
			if(mpsLines != null && mpsLines.size() > 0) {
				...
				for (int i = 0; i < workNumber; i++) {
					...
					if(ppmManager != null) {
						...
						generateManufactureOrder(mpsLine, pr);
	...
	private void generateManufactureOrder(MpsLine line, Requisition pr) {
		try {
...
//			pdmManager.verifyBOM(line.getMaterialRrn());
			...
			if(Env.getOrgRrn() ==139420L ){
				ppmManager.generateManufactureOrderTime(mps, line, pr, Env.getUserRrn());
			}else{
				ppmManager.generateManufactureOrder(mps, line, pr, Env.getUserRrn());
			}
		} catch(Exception e) {
