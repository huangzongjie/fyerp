...
public class PrepareTempSaleDataImportProgress2 extends SaleDataImportProgress {
	private static final Logger logger = Logger.getLogger(PrepareTempSaleDataImportProgress2.class);
	protected String tpsId;

	public PrepareTempSaleDataImportProgress2(Mps mps, String fileUrl, ADTable adTable,
...
//								}
//							}
							String excelValidate = cell.getStringCellValue();//标记为Y直接进入工作令的创建从
							if (excelValidate != null){
								if("Y".equals(excelValidate.toUpperCase())){
									tpsLine.setExcelValidate(true);//EXCEL校验，需要校验
									tpsLine.setSysValidate(false);//
								}
							}else{
								tpsLine.setSysValidate(true);
							}
						}else if(j == 12){
//							String excelValidate = cell.getStringCellValue();//标记为Y直接进入工作令的创建从
//							if (excelValidate != null){
