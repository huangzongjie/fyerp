...
public class TpsLinePrepareDialog extends TitleAreaDialog{
	private static final Logger logger = Logger.getLogger(TpsLinePrepareDialog.class);
	...
	public TpsLinePrepareDialog(Shell parentShell) {
		super(parentShell);
	}
	@Override
	protected Control createDialogArea(Composite parent) {
		this.setTitle(Message.getString("wip.create_mo_from_preparetpsline"));
...
//		EntityItemInput input = new EntityItemInput(tableManager.getADTable(), WHERE_CLAUSE+" and prepareTps = 'Y'", ORDER_BY);
	    String whereClause = getTpsWhereClause();
	    if(whereClause ==null) whereClause= "AND 1<> 1";
	    EntityItemInput input = new EntityItemInput(tableManager.getADTable(), 
	    		WHERE_CLAUSE+whereClause, ORDER_BY);
		viewer.setInput(input);
	    tableManager.updateView(viewer);
		
//    	List<TpsLine> prepareInputTpsLines = new ArrayList<TpsLine>();
//		try{
...
//	    	prepareInputTpsLines = progressDiglog.getPrepareTpsLines();
//	    }catch(Exception e ){
//	    	e.printStackTrace();
//	    }
//	    viewer.setInput(prepareInputTpsLines);
//	    tableManager.updateView(viewer);
...
//			toMo.setUpdatedBy(Env.getUserRrn());
			...
			toMo.setIsPrepareMo(true);//从有预处理物料开出来的计划，转为工作令都为预处理
		}
		return toMo;
	}
	...
	protected String getTpsWhereClause() {
		...
		try {
			//工作令创建从临时计划查询条件
			String tableName = "WIPManufactureOrderWhereClause2";
			ADManager entityManager = Framework.getService(ADManager.class);
			ADTable adTable = (ADTable)entityManager.getADTable(0L, tableName);
			whereClause = adTable.getWhereClause();
		} catch(Exception e) {
			logger.error("TpsLinePrepareDialog : initAdTableByTableId()", e);
		}
		return whereClause;
	}
