...
public class PurImportProgress  implements IRunnableWithProgress {
	...
	protected List<TmpHjdImport> hjdImports = new ArrayList<TmpHjdImport>();
	...
	protected void executeOneWorked(HSSFRow row, IProgressMonitor monitor, int index) {
		TmpHjdImport hjdImport = new TmpHjdImport();
		hjdImport.setOrgRrn(Env.getOrgRrn());
		Date now = Env.getSysDate();
		try {
			// 将excel中第i行的相关数据读入到salePlanLine中
			readExcelRow(row, hjdImport);
			...
			if (flag) {
				if ( hjdImport.getMaterialId()!=null) {
					hjdImports.add(hjdImport);
				} else {
	...
	protected void readExcelRow(HSSFRow row, TmpHjdImport hjdImport) {
		int j = 0;
					...
					case HSSFCell.CELL_TYPE_STRING:
						if(j == 0) {					
							...
							if (readMaterial != null) {
								hjdImport.setMaterialId(readMaterial.getMaterialId());
							} else {
								flag = false;
							...
							try {
								...
								if(hjdImport != null && hjdImport!=null){
									BigDecimal qty = BigDecimal.valueOf(l);
									hjdImport.setQty(qty);
								}else{
									flag = false;
...
//			} 
	...
	public List<TmpHjdImport> getHjdImports() {
		return hjdImports;
	}

	public void setHjdImports(List<TmpHjdImport> hjdImports) {
		this.hjdImports = hjdImports;
	}
}
