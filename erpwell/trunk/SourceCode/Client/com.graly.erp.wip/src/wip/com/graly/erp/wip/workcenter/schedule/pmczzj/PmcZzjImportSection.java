...
public class PmcZzjImportSection extends MasterSection {
	private static final Logger logger = Logger.getLogger(PmcZzjImportSection.class);
	...
	protected PmcZzjMaterialSection purMaterialSection;
	protected ToolItem itemWms;

	public PmcZzjImportSection(EntityTableManager tableManager,PmcZzjMaterialSection purMaterialSection) {
		super(tableManager);
		this.purMaterialSection = purMaterialSection;
		setWhereClause("1=1 and created >=trunc(sysdate-1)");//刚打开时显示空内容
	}
	...
	protected void uploadAdapter2() {
		try {
			...
			if (selectedFile != null) {
				...
				PmcZzjImportDialog progressDialog = new PmcZzjImportDialog(UI.getActiveShell());
				PmcZzjImportProgress progress = new PmcZzjImportProgress(null,
...
//						String whereClause = " to_char(scheduleDate,'yyyy-mm') = '"+date+"'" + " AND moId is null    and workcenterRrn="+this.getWorkCenterRrn();
						List<PmcZzjImport> wses = adManager.getEntityList(Env.getOrgRrn(),PmcZzjImport.class,Integer.MAX_VALUE, "1=1 and created>=trunc(sysdate) " ,null);
						for(PmcZzjImport ws : wses){
							adManager.deleteEntity(ws);//
						}
						for(PmcZzjImport chfImport : progress.getChfImports()){
