...
public class ByLotVirtualHouseInputQtySection  {
	private static final Logger logger = Logger.getLogger(ByLotVirtualHouseInputQtySection.class);
	...
	protected ByLotVirtualHouseInputQtyDialog parentDialog;
	...
	public ByLotVirtualHouseInputQtySection(ADTable adTable, ByLotVirtualHouseInputQtyDialog parentDialog) {
...
//				MovementWorkShopLineLot lineLot  = pareseMovementLineLot(l, BigDecimal.ONE, lot);
				...
				VirtualHouseQtySetupDialog qtyDialog = new VirtualHouseQtySetupDialog(UI.getActiveShell(),  l, lot, null);
				int openId = qtyDialog.open();
				if(openId == Dialog.OK) {
					lineLot = pareseMovementLineLot(l, qtyDialog.getInputQty(), lot);
				} else if(openId == Dialog.CANCEL) 
					return;
				qtyTotal= qtyTotal.add(qtyDialog.getInputQty());
...
//					库存不足校验后期看看是否需要
//					Storage storage =invManager.getMaterialStorage(Env.getOrgRrn(),  line.getMaterialRrn(), 151046L, Env.getUserRrn());
		...
		try {
        	...
        	if (items != null && items.length > 0){
        		...
        		if(obj != null && obj instanceof MovementWorkShopLineLot) {
        			...
        			if (confirmDelete) {
        				MovementWorkShopLineLot linelot = (MovementWorkShopLineLot) obj;
        				getLineLots().remove(obj);
        				qtyTotal= qtyTotal.subtract(linelot.getQtyMovement());
        				txtQtyTotal.setText(qtyTotal.toString());
        				refresh();
