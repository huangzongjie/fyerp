...
public class MoLineSection implements IRefresh {
	private static final Logger logger = Logger.getLogger(MoLineSection.class);
	...
	private ToolItem itemPreviewTask;
	...
	public MoLineSection(TableListManager tableManager) {
		this.tableManager = tableManager;
	}
	
	public MoLineSection(TableListManager tableManager, EntityTableManager entityTableManager) {
		this.tableManager = tableManager;
...
//				msg.append(WORK_CENTER);
	...
	public void createToolBar(Section section) {
		...
		createToolItemWorkShopSchedule(toolBar);
		...
		createToolItemPreviewTask(toolBar);
		setInitAuthority(hasWCAuthority && true);
		section.setTextClient(toolBar);
	}
	
	//按钮实现
	protected void createToolItemWorkShopSchedule(ToolBar tBar) {
		itemWorkShopSchedule = new ToolItem(tBar, SWT.PUSH);
		itemWorkShopSchedule.setText("车间排程");
		itemWorkShopSchedule.setImage(SWTResourceCache.getImage("split"));
		itemWorkShopSchedule.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent event) {
				workShopScheduleAdapter();
			}
		});
	}
...
//				}
	...
	public void refresh() {
		try {
			...
			List<ManufactureOrderLine> list = wipManager.getMoLineByWorkCenter(Env.getOrgRrn(),
...
//		return this.whereClause;
	...
	protected void workShopScheduleAdapter() {
		try {
			if (selectedMoLine != null) {
				
				String dialogTableName = "WorkShopSchedule";
				if(selectedMoLine.getMasterMoRrn() != null && !selectedMoLine.getMasterMoRrn().equals(selectedMoLine.getObjectRrn())) {
					...
					String moId="moId ='"+selectedMoLine.getMasterMoId()+"'";
					WorkShopSchedduleDialog prepareDialog = new WorkShopSchedduleDialog(UI.getActiveShell(),dialogADTable,moId, selectedMoLine, dialogADTable);
	...
	private void createToolItemPreviewTask(ToolBar bar) {
		if(Env.getOrgRrn()==70000000L|| Env.getOrgRrn() == 49204677L){
			itemPreviewTask = new ToolItem(bar, SWT.PUSH);
	...
	protected void previewTaskAdapter() {
		try {
			String report = "motask_report_hy.rptdesign";
			HashMap<String, Object> params = new HashMap<String, Object>();
			params.put(ReportUtil.SERVLET_NAME_KEY, ReportUtil.VIEWER_FRAMESET);
			HashMap<String, String> userParams = new HashMap<String, String>();
			Long moRrn = 0L;
			if(selectedMoLine != null){
				moRrn = selectedMoLine.getObjectRrn();
			}else{
				UI.showError(Message.getString("mo.no_mo_selected"));
				return;
			}
