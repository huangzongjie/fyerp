...
//		return false;
	...
	public void initMapLot(){
		...
		try {
			...
			wc.setObjectRrn(selectMoLine.getWorkCenterRrn());
			wc = (WorkCenter) adManager.getEntity(wc);
			selectMoLine.setWorkCenterId(wc.getName());
			if(selectMoLine.getMasterMoId()!=null&&!"".equals(selectMoLine.getMasterMoId())){
				ManufactureOrder mo = new ManufactureOrder();
				mo.setObjectRrn(selectMoLine.getMasterMoRrn());
				mo = (ManufactureOrder) adManager.getEntity(mo);
//				selectMoLine.setCustomerName(customerName);
//				selectMoLine.setOrderId(orderId);
			}
			...
			if(selectMoLine.getMasterMoId()!=null&&!"".equals(selectMoLine.getMasterMoId())){
				List<Lot> lots = wipManager.getAvailableLot4In(selectMoLine.getMasterMoRrn());
				for(Lot lot: lots){
