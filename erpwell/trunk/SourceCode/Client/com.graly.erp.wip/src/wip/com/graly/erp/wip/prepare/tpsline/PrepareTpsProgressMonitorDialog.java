 ...
 *校验临时计划是否有预处理的进度条 
 * */
public class PrepareTpsProgressMonitorDialog extends ProgressMonitorDialog {
	private static final Logger logger = Logger.getLogger(PrepareTpsProgressMonitorDialog.class);
	...
	private List<TpsLine> tpsLines;
//	private List<TpsLine> allTpsLines;

	public PrepareTpsProgressMonitorDialog(Shell parent,List<TpsLine> tpsLines) {
		super(parent);
		this.tpsLines = tpsLines;
	}
	
	public PrepareTpsProgressMonitorDialog(Shell parent, String title,List<TpsLine> tpsLines) {
		super(parent);
		this.title = title;
		this.tpsLines = tpsLines;
	}
...
//				allTpsLines = new ArrayList<TpsLine>();
				...
				for (int i = 0; i < workNumber; i++) {
					...
					if(tpsLines.get(i) != null) {
						...
						try{
							prepareTps = wipManager.verifyPrepareTps(Env.getOrgRrn(),Env.getUserRrn(), tpsLines.get(i));
						}catch(Exception e ){
...
//							errorMaterials.add(materials.get(i));
						...
						if(prepareTps) {
							tpsLines.get(i).setPrepareTps(true);
