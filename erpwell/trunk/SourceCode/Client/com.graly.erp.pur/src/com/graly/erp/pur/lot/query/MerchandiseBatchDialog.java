...
public class MerchandiseBatchDialog extends ExtendDialog {
	...
	protected void setQueryDialogToSection() {
		if(getParent() instanceof EntityEditor){
			...
			if(editor.getActivePageInstance() instanceof MerchandiseBatchPage){
				MerchandiseBatchPage page = (MerchandiseBatchPage) editor.getActivePageInstance();
				MerchandiseBatchSection section = page.getMbatchSection();
			//	queryDialog.setIRefresh(section);
				section.setQueryDialog(queryDialog);
				((MerchandiseBatchSection)section).setExtendDialog(this);
			}
		}
	}
	...
	class PrInternalQueryDialog extends EntityQueryDialog {
		...
		protected void okPressed() {
			MerchandiseBatchEditor editor = (MerchandiseBatchEditor) getParent();
			MerchandiseBatchPage page = (MerchandiseBatchPage) editor.getActivePageInstance();
			MerchandiseBatchSection section = page.getMbatchSection();
			section.setQueryLotId(this.queryForm.getFields().get("lotId").getValue().toString());
			section.refresh();
//			okPressed();
			setReturnCode(OK);
//			iRefresh.refresh();
