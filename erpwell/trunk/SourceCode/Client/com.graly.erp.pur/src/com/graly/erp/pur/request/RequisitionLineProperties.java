...
public class RequisitionLineProperties extends ChildEntityProperties {

	private static final Logger logger = Logger.getLogger(RequisitionLineProperties.class);
	...
	private static final String	FieldName_ProductNo	= "productNo";//原能产品货号

	protected ToolItem itemClose;

	public RequisitionLineProperties() {
		super();
	}

	public RequisitionLineProperties(EntityBlock masterParent, ADTable table, Object parentObject) {
	...
	public void createToolBar(Section section) {
		...
		new ToolItem(tBar, SWT.SEPARATOR);
		createToolItemClosePRLine(tBar);
		section.setTextClient(tBar);
	}

	protected void createToolItemClosePRLine(ToolBar tBar) {
	...
	protected void closePRLineAdapter() {
		try {
		...
		try {
			...
			if (getAdObject() != null) {
				...
				if (saveFlag) {
					...
					RequisitionLine prLine = purManager.savePRLine((RequisitionLine) getAdObject(), Env.getUserRrn());
	...
	protected void changedMasterParentStatus() {
		RequisitionLineEntityBlock parentBlock = (RequisitionLineEntityBlock) getMasterParent();
	...
	public ADBase createAdObject() throws Exception {
		...
		try {
			RequisitionLineEntityBlock block = (RequisitionLineEntityBlock) this.getMasterParent();
			...
			try {
				if(material != null && material.getObjectRrn() != null){
					...
					if (vendorMaterial != null ) {
						...
						rtfProductNo.setValue(vendorMaterial.getProductNo());
					} else {
						rtf.setValue(null);
						rtfProductNo.setValue(null);
					}
					rtf.refresh();
					rtfProductNo.refresh();
				}
			} catch (Exception e) {
				ExceptionHandlerManager.asyncHandleException(e);
				return;
