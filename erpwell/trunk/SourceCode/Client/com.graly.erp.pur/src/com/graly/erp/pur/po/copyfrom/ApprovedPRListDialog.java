...
public class ApprovedPRListDialog extends InClosableTitleAreaDialog {
	private static final Logger logger = Logger.getLogger(ApprovedPRListDialog.class);
	private static int MIN_DIALOG_WIDTH = 700;
	private static int MIN_DIALOG_HEIGHT = 450;
	private static String TABLE_NAME = "PURRequisition";
	
	protected ADTable table;
	protected ManagedForm managedForm;
	protected ApprovedPRListSection prListSection;
	protected PurchaseOrder po;

	public ApprovedPRListDialog(Shell parent, PurchaseOrder po) {
        super(parent);
        this.po = po;
    }
	
	public ApprovedPRListDialog(Shell parent, ADTable table, PurchaseOrder po){
		this(parent, po);
		this.table = table;
	}
	...
	protected void createFormContent(Composite composite) {
		...
		prListSection = new ApprovedPRListSection(new EntityTableManager(table), managedForm);
		prListSection.setWhereClause(" docStatus in ('APPROVED') ");
		prListSection.setApprovedPRDialog(this);
