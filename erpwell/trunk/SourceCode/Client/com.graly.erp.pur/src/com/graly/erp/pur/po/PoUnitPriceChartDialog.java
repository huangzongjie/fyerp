 ...
 * 同时为了系统性能应该添加一个时间段
 * */
public class PoUnitPriceChartDialog extends EntityQueryDialog{
	private String reportName;
	protected ADTable printADTable;
	protected Long objectRrn;
	public PoUnitPriceChartDialog(Shell parent,
			EntityTableManager tableManager, IRefresh refresh,Long objectRrn) {
        ...
        try {
    		...
    		for(String name : keys.keySet()){
    			if("approvedDate".equals(name)){
    				...
    				if(fromDate != null){
    					userParams.put("PO_STARTDATE", I18nUtil.formatDate(fromDate));
    				}
    				
    				if(toDate != null){
    					userParams.put("PO_ENDDATE", I18nUtil.formatDate(toDate));
    				}
    			}	
    		}
