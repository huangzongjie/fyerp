...
public class FlowWizardDialog extends InClosableTitleAreaDialog implements IWizardContainer2,
		IPageChangeProvider {
	 ...
	 */
	public FlowWizardDialog(Shell parentShell, FlowWizard newWizard) {
		super(parentShell);
		newWizard.setDialog(this);
	...
	private Button createCancelButton(Composite parent) {
		...
		button.setText(Message.getString("common.cancel"));//IDialogConstants.CANCEL_LABEL
	 ...
	 */
	private Composite createNextButton(Composite parent) {
		// increment the number of columns in the button bar
		((GridLayout) parent.getLayout()).numColumns++;
		...
		nextButton = createButton(composite, IDialogConstants.NEXT_ID,
				Message.getString("common.next"), false);
		return composite;
	}
	
	private Composite createPreviousButton(Composite parent) {
		((GridLayout) parent.getLayout()).numColumns++;
		Composite composite = new Composite(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.numColumns = 0; // will be incremented by createButton
		...
		backButton = createButton(composite, IDialogConstants.BACK_ID,
				Message.getString("common.back"), false);
		return composite;
	}
	...
	protected void backPressed() {
		if (currentPage instanceof FlowWizardPage) {
			String nextPage = ((FlowWizardPage)currentPage).doPrevious();
			if(nextPage != null && !"".equals(nextPage)) {
				IWizardPage page = wizard.getPage(nextPage);
				if (page == null) {
	...
	public void nextPressed() {
		if (currentPage instanceof FlowWizardPage) {
			String nextPage = ((FlowWizardPage)currentPage).doNext();
			if (FINISH.equalsIgnoreCase(nextPage)) {
			...
			//在此判断是否跳过该页面，如果是则将page设为当前页
			if (((FlowWizardPage)page).isJumpOver()) {
				IWizardPage prePage = currentPage;
				if(prePage.getControl() != null)
	...
	private void updateForPage(IWizardPage page) {
		...
		if (page.getControl() == null) {
			...
			// 每当通过上一步再返回到该页面时(此时page.control已经创建)，刷新该页面的内容，因为用户在上个页面的操作
			// 可能已经影响到了本页面的显示内容(上个页面操作改变的值会放在IWizardContext中)。在单击"上一步"也会调用
			// updatePageContent方法
			if(page instanceof FlowWizardPage) {
				((FlowWizardPage)page).updatePageContent();
			}
		}
		...
		if (oldPage != null && oldPage.getControl() != null) {
	...
	private void showStartingPage() {
		...
		if (currentPage != null) {
			currentPage.setVisible(true);
			if (currentPage instanceof FlowWizardPage) {
				FlowWizardPage fPage = ((FlowWizardPage)currentPage);
				fPage.refresh();
			}
			// update the dialog controls
			update();
	...
	/*
	 * update Button Text "Next>" to "Finish" when go to the finally step
	 */
	public void updateButtonName(int buttonId, String newName) {
		getButton(buttonId).setText(newName);
	}
}
