...
public final class XUrlComposite extends Composite {
	Text text;
	Button arrow;
	boolean hasFocus;
...
public XUrlComposite (Composite parent, int style) {
	...
	arrow.setImage(SWTResourceCache.getImage("url-button"));
	...
	listener = new Listener () {
		public void handleEvent (Event event) {
			...
			if (XUrlComposite.this == event.widget) {
				urlEvent (event);
				return;
			}
	...
	filter = new Listener() {
		public void handleEvent(Event event) {
			...
			if (shell == XUrlComposite.this.getShell ()) {
				handleFocus (SWT.FocusOut);
			}
		}
	};
	
	int [] urlEvents = {SWT.Dispose, SWT.Move, SWT.Resize};
	for (int i=0; i<urlEvents.length; i++) this.addListener (urlEvents [i], listener);
