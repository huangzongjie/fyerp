 ...
 * @see Widget#getStyle()
 */
public XCombo (Composite parent, TableViewer tableViewer, String keyColumn, String valueColumn, int style, boolean isReadOnly) {
	super (parent, style = checkStyle (style, parent));
//	getGridLayout().horizontalSpacing = 2;
	...
	if (isReadOnly()) textStyle |= SWT.READ_ONLY;
...
public XCombo (Composite parent, TableViewer tableViewer, String keyColumn, String valueColumn, int style) {
	this(parent, tableViewer, keyColumn, valueColumn, style, false);
}
...
//	width = Math.max (wHint, Math.max (maxImageSize.x + imageTextSpace + textWidth + 2*spacer + arrowSize.x + 2*borderWidth, listSize.x));
	width = Math.max (wHint, Math.max (maxImageSize.x + imageTextSpace + textWidth + 2*spacer + arrowSize.x + 2*borderWidth, 0));
	return new Point (width, height);
}
...
void dropDown (boolean drop) {
	...
	int itemHeight = table.getItemHeight () * visibleItemCount;
...
void listEvent (Event event) {
	switch (event.type) {
		...
		case SWT.Selection: {
			...
			try{
				String keyStr = (String)PropertyUtil.getPropertyForString(obj, keyColumn);
				text.setData(keyStr);
			} catch (Exception e){
				e.printStackTrace();
				text.setData(null);
...
public void remove (String string) {
	...
	table.remove (getTableItemIndexByKey(string));
}

protected TableItem getTableItemByKey(String key) 
{
	int itemCount = table.getItemCount();
...
protected TableItem getTableItemByValue(String value) 
{
	int itemCount = table.getItemCount();
	for (int i=0; i<itemCount; i++) 
	{
...
protected int getTableItemIndexByKey(String key) 
{
	int itemCount = table.getItemCount();
	for (int i=0; i<itemCount; i++) 
	{
...
protected int getTableItemIndexByValue(String value) 
{
	int itemCount = table.getItemCount();
	for (int i=0; i<itemCount; i++) 
	{
...
public void setText (String string, boolean replaceFlag) {
	checkWidget();
//	if (string == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
//	int index = table.indexOf (getTableItem(string));
	TableItem tableItem = getTableItemByValue(string);
	int index = -1;
	if (tableItem != null)		
		index = table.indexOf (getTableItemByValue(string));	
	if (index == -1) {
		table.deselectAll ();
		if (replaceFlag) {
			text.setText (string);
...
public void setKey (String key) {
	...
	TableItem tableItem = getTableItemByKey(key);
	int index = -1;
	if (tableItem != null)		
		index = table.indexOf (getTableItemByKey(key));	
	if (index == -1) {
		table.deselectAll ();
		text.setText ("");
		text.setData("");
...
public Object getData () {
	if (this.getKey() != null) {
		TableItem tableItem = getTableItemByKey(this.getKey());
		if (tableItem != null) {
