...
public class EntityBlockDialog extends InClosableTitleAreaDialog {
	
	private static final Logger logger = Logger.getLogger(EntityBlockDialog.class);
	...
	public EntityBlockDialog(Shell parent) {
        super(parent);
    }
	
	public EntityBlockDialog(Shell parent, ADTable table){
		this(parent);
		this.table = table;
	}
	...
	protected void fireSelectionChanged(IManagedForm managedForm){
		try{
			...
			// 实现选中列表中第一行
			if(((TableViewer)block.getViewer()).getTable().getItemCount() > 0) {
				TableItem it = ((TableViewer)block.getViewer()).getTable().getItem(0);
